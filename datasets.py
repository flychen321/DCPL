...
class HSVTripletFolder(datasets.ImageFolder):
    def __init__(self, root, transform, h_num=4, train=True, debug=0):
        super(HSVTripletFolder, self).__init__(root, transform)
        self.h_num = h_num
        self.labels = np.array(self.imgs)[:, 1]
        self.data = np.array(self.imgs)[:, 0]
        self.labels_set = set(self.labels)
        ...
        self.h_ratio = np.linspace(0, 1.0, self.h_num + 1)
        self.s_ratio = np.arange(0.8, 1.0, 0.05).tolist() + np.arange(1.05, 1.2, 0.05).tolist()
        self.v_ratio = np.arange(0.5, 1.0, 0.05).tolist() + np.arange(1.05, 1.5, 0.05).tolist()
    ...
    def _get_pos_sample(self, label, index):
        ...
        for i in range(4):
            t = i % len(rand)
            tmp_index = pos_index[rand[t]]
            result_path.append(self.imgs[tmp_index][0])
        return result_path
    ...
    def _get_neg_sample(self, label):
        ...
        neg_index = neg_index.flatten()
        rand = random.randint(0, len(neg_index) - 1)
        return self.imgs[neg_index[rand]]
    ...
    def __getitem__(self, index):
        ...
        img_original.append(pos3)
        img_all_temp = []
        label_all = torch.Tensor(len(img_original), self.h_num).long()


        for i in range(len(img_original)):
            img_sub = []
            img_hsv = cv2.cvtColor(np.asarray(img_original[i]), cv2.COLOR_RGB2HSV)
            h_org, s_org, v_org = cv2.split(img_hsv)
            for j in range(self.h_num):
                h = (((h_org.astype(np.uint16) + self.h_ratio[j] * 180) * (256 / 180.0)).astype(np.uint8)) * (180.0 / 256)
                if np.random.rand() > self.prob_s:
                    s = s_org * np.random.choice(self.s_ratio)
                    ret, s = cv2.threshold(s, 255, 255, cv2.THRESH_TRUNC)
                    s_org = s.astype(np.uint8)
                if np.random.rand() > self.prob_v:
                    v = v_org * np.random.choice(self.v_ratio)
                    ret, v = cv2.threshold(v, 255, 255, cv2.THRESH_TRUNC)
                    v_org = v.astype(np.uint8)
                img_h = cv2.merge([h.astype(np.uint8), s_org, v_org])
                img = Image.fromarray(cv2.cvtColor(img_h, cv2.COLOR_HSV2RGB))
                img_sub.append(img)
                label_all[i][j] = self.class_num * j + int(label)
            img_all_temp.append(img_sub)

        img_all = torch.Tensor(len(img_all_temp), self.h_num, 3, 256, 128).zero_()
        if self.transform is not None:
            for i in range(len(img_all_temp)):
                for j in range(self.h_num):
                    img_all[i][j] = self.transform(img_all_temp[i][j])
        ...
        if self.label_to_indices[label].shape[0] > 1:
            index_2 = np.random.choice(list(set(self.label_to_indices[label]) - set([index])), 1, replace=False)
        else:
            index_2 = np.random.choice(self.label_to_indices[label], 1, replace=False)
        # label2 = np.random.choice(list(self.labels_set - set([label])))
        # index_2 = np.random.choice(self.label_to_indices[label2], 1)
        img2, label2 = self.data[index_2[0]], self.labels[index_2[0]].item()
        img2 = default_loader(img2)
        img_original = []
        img_original.append(img2)
        label_original = []
        label_original.append(label2)
        img_all_temp = []
        label_all2 = torch.Tensor(len(img_original), self.h_num).long()
        for i in range(len(img_original)):
            img_sub = []
            label_sub = []
            label = label_original[i]
            ...
            for j in range(self.h_num):
                ...
                label_all2[i][j] = self.class_num * j + int(label)
            img_all_temp.append(img_sub)
        img_all2 = torch.Tensor(len(img_original), self.h_num, 3, 256, 128)
        if self.transform is not None:
            for i in range(len(img_all_temp)):
                for j in range(self.h_num):
                    img_all2[i][j] = self.transform(img_all_temp[i][j])
        ...
        if self.train:
            return r_img, r_pos, img_all2[0], \
                   label_all[0], label_all[0], label_all2[0]
        else:
            return r_img
