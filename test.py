...
from datasets import HSVTripletFolder
######################################################################
# Options
# --------
...
def test_function(test_dir=None, net_loss_model=None, domain_num=None, which_epoch=None):
    ...
    # image_datasets = {x: HSVTripletFolder(os.path.join(data_dir, x), data_transforms, h_num=opt.domain_num, train=False) for x in dataset_list}
    ...
    def extract_all_features(model, dataloaders, domain_num=5):
        features = torch.FloatTensor()
        for data in dataloaders:
            img_all = data
            n, domain_num, c, h, w = img_all.size()
            ff_dd = torch.FloatTensor()
            ff_ss = torch.FloatTensor()
            for d in range(2):
                img = img_all[:, d]
                ff_d = torch.FloatTensor(n, 512).zero_().cuda()
                ...
                ff_dd = torch.cat((ff_dd, ff_d.detach().cpu().float()), 1)
                ff_ss = torch.cat((ff_ss, ff_s.detach().cpu().float()), 1)
            features = torch.cat((features, torch.cat((ff_dd, ff_ss), 1)), 0)
        return features
    ...
    with torch.no_grad():
        for i in range(len(dataset_list)):
            ...
            # dataset_feature.append(extract_all_features(model, dataloaders[dataset_list[i]], opt.domain_num))
